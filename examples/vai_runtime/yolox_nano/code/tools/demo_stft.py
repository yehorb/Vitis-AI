...
def make_parser():
    ...
    parser.add_argument(
        "-f", "--exp_file", required=True, type=str,
        help="Experiment description file"
    )
    parser.add_argument(
        "-c", "--ckpt", required=True, type=str,
...
def run_inference(model, img: np.ndarray, exp, conf_thre: float, nms_thre: float, device: str):
    """
...
def visualize_predictions(
    ...
    ax.set_title(f"Tile: {tile_id} | GT: {n_gt} | Pred: {n_pred} (conf>{conf_thre:.2f})")
...
def main():
    ...
    for idx in indices:
        ...
        predictions = run_inference(
            model, img, exp, args.conf, args.nms, args.device
        )
        ...
        visualize_predictions(
            img, gt_boxes, predictions, tile_id, args.conf, save_path
        )
